...
if [ $RUNTIME = "knative" ]; then
  ...
  helm install istio ${istio_chart} --namespace istio-system --wait --set gateways.istio-ingressgateway.type=${K8S_SERVICE_TYPE}
...
helm install riff ${riff_chart} --namespace riff-system --wait \
